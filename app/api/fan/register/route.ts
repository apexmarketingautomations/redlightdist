...
export async function POST(request:Request){
  const tenant=await requestTenant(request);if(!tenant)return new NextResponse("Unknown creator site",{status:404});const parsed=schema.safeParse(Object.fromEntries(await request.formData()));if(!parsed.success)return fanRedirect(request,"/fan/register?error=invalid");const {email,password}=parsed.data;
  try{
    await withCreator(tenant.creatorId,async client=>{
      ...
      await dispatchAutomationEvent(client,{key:"fan.registered",creatorId:tenant.creatorId,fanId:fan.id,payload:{emailConsent:consent},occurredAt:new Date()});
    });
