...
const schema=z.object({creatorId:z.string().uuid(),campaignId:z.string().uuid(),operation:z.enum(["send","schedule","pause","resume","cancel"]),scheduledFor:z.string().trim().optional()});
...
export async function campaignAction(_state:FormState,form:FormData):Promise<FormState>{
  ...
  });}catch(error){const m=error instanceof Error?error.message:"";console.error("Campaign action failed",{creatorId:d.creatorId,campaignId:d.campaignId,message:m});return{ok:false,message:m==="INVALID_SCHEDULE"?"Choose a future schedule time.":m==="CAMPAIGN_CLOSED"?"This campaign is already closed.":"Campaign update could not be completed."};}
  revalidatePath(`/dashboard/${d.creatorId}/marketing`);return{ok:true,message:d.operation==="schedule"?"Campaign scheduled.":d.operation==="send"?"Campaign queued for delivery.":"Campaign updated."};
}
