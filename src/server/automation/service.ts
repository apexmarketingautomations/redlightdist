...
export async function processAutomationQueue(maxJobs=10){
  const workerId=crypto.randomUUID();let processed=0;
  for(let i=0;i<Math.max(1,Math.min(50,maxJobs));i++){
    const claim=(await db.query<{step_id:string;creator_id:string}>("SELECT step_id,creator_id FROM claim_automation_step($1)",[workerId])).rows[0];if(!claim)break;
    try{
      await withCreator(claim.creator_id,async client=>{
        const step=(await client.query<{id:string;run_id:string;action_key:string;config:Record<string,unknown>;fan_id:string|null;event_payload:Record<string,unknown>}>(`SELECT s.id,s.run_id,s.action_key,s.config,r.fan_id,r.event_payload FROM automation_run_steps s JOIN automation_runs r ON r.creator_id=s.creator_id AND r.id=s.run_id WHERE s.creator_id=$1 AND s.id=$2 FOR UPDATE`,[claim.creator_id,claim.step_id])).rows[0];if(!step)throw new Error("AUTOMATION_STEP_NOT_FOUND");await client.query("UPDATE automation_runs SET status='running',started_at=coalesce(started_at,now()) WHERE creator_id=$1 AND id=$2",[claim.creator_id,step.run_id]);await executeStep(client,{creatorId:claim.creator_id,fanId:step.fan_id,actionKey:step.action_key,config:step.config,eventPayload:step.event_payload});await client.query("UPDATE automation_run_steps SET status='completed',completed_at=now() WHERE creator_id=$1 AND id=$2",[claim.creator_id,step.id]);const pending=await client.query("SELECT 1 FROM automation_run_steps WHERE creator_id=$1 AND run_id=$2 AND status<>'completed' LIMIT 1",[claim.creator_id,step.run_id]);if(!pending.rowCount)await client.query("UPDATE automation_runs SET status='completed',completed_at=now() WHERE creator_id=$1 AND id=$2",[claim.creator_id,step.run_id]);
      });processed++;
    }catch(error){const message=error instanceof Error?error.message:"unknown";await withCreator(claim.creator_id,async client=>{await client.query("UPDATE automation_run_steps SET status=CASE WHEN attempts>=5 THEN 'failed' ELSE 'failed' END,last_error=$3 WHERE creator_id=$1 AND id=$2",[claim.creator_id,claim.step_id,message.slice(0,1000)]);const row=(await client.query<{run_id:string}>("SELECT run_id FROM automation_run_steps WHERE creator_id=$1 AND id=$2",[claim.creator_id,claim.step_id])).rows[0];if(row)await client.query("UPDATE automation_runs SET status='failed',error=$3 WHERE creator_id=$1 AND id=$2",[claim.creator_id,row.run_id,message.slice(0,1000)]);});}
  }
  return processed;
}
