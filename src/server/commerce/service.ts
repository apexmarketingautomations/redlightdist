...
import { getPaymentProvider, type PaymentProvider, type ProviderContext } from "@/src/modules/payments/provider";
...
function context(creatorId:string,account:AccountRow):ProviderContext{
  return {creatorId,accountId:account.id,merchantReference:account.merchant_reference};
}
...
export async function beginSubscription(client:PoolClient,input:{creatorId:string;fanId:string;tierId:string;returnUrl:string}){
  ...
  const account=await defaultAccount(client,input.creatorId);
  const provider=getPaymentProvider(account.provider);
