...
export type CCBillConfig={
  flexFormId:string;
  clientSubacc:string;
  encryptionKey:string;
  currencyCodes?:Record<string,string>;
};
export type SegpayConfig={
  merchantId:string;
  srsUserId:string;
  srsAccessKey:string;
  eticketId?:string;
  subscriptionEticketId?:string;
  purchaseEticketId?:string;
  joinBaseUrl?:string;
  dynamicPricingBaseUrl?:string;
...
function load():Record<string,PaymentAccountConfig>{
  ...
  try{
    ...
    cache=parsed as Record<string,PaymentAccountConfig>;
    return cache;
