...
export function assertMediaType(contentType:string,allowed:readonly string[]):void{if(!allowed.includes(contentType.toLowerCase()))throw new Error("UNSUPPORTED_MEDIA_TYPE");}
export function assertUploadSize(byteSize:number,limit:number):void{if(!Number.isSafeInteger(byteSize)||byteSize<=0||byteSize>limit)throw new Error("UPLOAD_SIZE_LIMIT");}

const registry=new Map<string,StorageProvider>();
export function registerStorageProvider(provider:StorageProvider){registry.set(provider.name,provider);}
export function getStorageProvider(name:string){const provider=registry.get(name);if(!provider)throw new Error(`STORAGE_PROVIDER_NOT_CONFIGURED:${name}`);return provider;}
export function hasStorageProvider(name:string){return registry.has(name);}

/** Application-level fallback token for protected gateways. Native storage presigning is preferred. */
...
export function verifyMediaGrant(token:string,secret:string,nowSeconds=Math.floor(Date.now()/1000)):{creatorId:string;assetId:string}|null{
  ...
  const [creatorId,assetId,expiresRaw,signature]=parts;const expiresAt=Number(expiresRaw);
  if(!creatorId||!assetId||!Number.isSafeInteger(expiresAt)||expiresAt<=nowSeconds||!signature)return null;
  const payload=`${creatorId}.${assetId}.${expiresAt}`;const expected=createHmac("sha256",secret).update(payload).digest("base64url");
