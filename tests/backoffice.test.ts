...
beforeAll(async () => {
  for (const name of [
    ...
    await db.exec(
      (await readFile(`migrations/${name}`, "utf8")).replace(
        "CREATE EXTENSION IF NOT EXISTS pgcrypto;",
        "",
      ),
    );
  for (const [id, email, isAdmin, token] of [
    [admin, "admin@example.test", true, "admin-token"],
    [a, "a@example.test", false, "a-token"],
    [b, "b@example.test", false, "b-token"],
  ] as const) {
    await db.query(
      "INSERT INTO platform_users(id,email,password_hash,is_platform_admin) VALUES($1,$2,'unused',$3)",
      [id, email, isAdmin],
    );
    await db.query(
  ...
  await db.query(
    "INSERT INTO creators(id,name,slug,status) VALUES($1,'A','a','active'),($2,'B','b','active')",
    [ca, cb],
  );
  await db.query(
    "INSERT INTO creator_users(creator_id,user_id,role) VALUES($1,$2,'owner'),($3,$4,'owner')",
    [ca, a, cb, b],
  );
  await db.query(
    "INSERT INTO creator_settings(creator_id,bio) VALUES($1,'A private'),($2,'B private')",
    [ca, cb],
  );
  await db.exec(
    "CREATE ROLE redlight_runtime; GRANT USAGE ON SCHEMA public TO redlight_runtime; GRANT SELECT,INSERT,UPDATE,DELETE ON ALL TABLES IN SCHEMA public TO redlight_runtime; SET ROLE redlight_runtime;",
  );
...
describe("back office authorization with real SQL and a restricted database role", () => {
  it("gives platform administrators audited client management without client membership", async () => {
    ...
    const rows = await withPlatformAdmin(
      async (client) =>
        (
          await client.query(
            "SELECT creator_id,bio FROM creator_settings ORDER BY creator_id",
          )
        ).rows,
    );
      ...
      (
        await withPlatformAdmin(
          async (client) =>
            (
              await client.query(
                "SELECT action FROM audit_logs WHERE creator_id=$1",
                [cb],
              )
            ).rows,
        )
      )[0]?.action,
    ).toBe("workspace.settings.updated");
  });
  it("prevents clients from selecting administrator access", async () => {
    ...
    await expect(
      settingsAction(
        initial,
        form({ admin: "true", creatorId: cb, name: "Stolen", bio: "No" }),
      ),
    ).rejects.toThrow("REDIRECT:/dashboard");
    await expect(
      adminAction(
        initial,
        form({ operation: "user-role", userId: a, role: "admin" }),
      ),
    ).rejects.toThrow("REDIRECT:/dashboard");
  });
  it("allows own workspace access and rejects cross-tenant reads and writes", async () => {
    state.token = "a-token";
    const rows = await withCreatorUser(
      a,
      ca,
      async (client) =>
        (await client.query("SELECT bio FROM creator_settings")).rows,
    );
    expect(rows).toEqual([{ bio: "A private" }]);
    await expect(
      withCreatorUser(a, cb, async (client) =>
        client.query("SELECT bio FROM creator_settings"),
      ),
    ).rejects.toThrow("CREATOR_ACCESS_DENIED");
    await expect(
      settingsAction(
        initial,
        form({ creatorId: cb, name: "Stolen", bio: "No" }),
      ),
    ...
    expect(
      (
        await settingsAction(
          initial,
          form({ creatorId: ca, name: "A updated", bio: "Own update" }),
        )
      ).ok,
    ).toBe(true);
  });
  it("denies settings writes to a read-only workspace role", async () => {
    ...
    await expect(
      settingsAction(
        initial,
        form({ creatorId: ca, name: "Unauthorized", bio: "No" }),
      ),
    ).rejects.toThrow("Workspace owner or administrator required");
  });
  it("creates users and workspaces, and protects the last workspace owner", async () => {
    ...
    expect(
      (
        await adminAction(
          initial,
          form({ operation: "remove-member", creatorId: ca, userId: a }),
        )
      ).message,
  ...
  it("blocks suspended workspaces and disabled sessions", async () => {
    ...
    await expect(withCreatorUser(a, ca, async () => true)).rejects.toThrow(
    ...
    expect(
      (
        await adminAction(
          initial,
          form({ operation: "user-status", userId: b, status: "disabled" }),
        )
      ).ok,
    ).toBe(true);
    state.token = "b-token";
  ...
  it("clears admin transaction context before a client operation", async () => {
    ...
    const result = await db.query(
      "SELECT current_setting('app.platform_admin',true) AS admin,current_setting('app.creator_id',true) AS tenant",
    );
    expect(result.rows[0]).toEqual({ admin: "", tenant: "" });
    expect(
      (await db.query("SELECT * FROM creator_settings")).rows,
    ).toHaveLength(0);
  });
});
