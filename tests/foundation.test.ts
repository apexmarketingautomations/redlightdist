...
import {
  normalizeHostname,
  resolveTenant,
} from "../src/modules/tenants/hostname";
...
describe("entitlement policy", () => {
  it("excludes advanced capabilities from Starter", () => {
    for (const feature of features) {
      expect(canUseFeature({ ...base, feature })).toBe(
        [
          "posts",
          "subscriptions",
          "customDomains",
  ...
  it("separates Pro streaming from Elite paid events", () => {
    ...
    expect(canUseFeature({ ...base, plan: "pro", feature: "livePpv" })).toBe(
      false,
    );
    expect(canUseFeature({ ...base, plan: "elite", feature: "livePpv" })).toBe(
      true,
    );
  });
  it("cannot override suspension", () => {
    ...
    expect(
      canUseFeature({ ...base, billingStatus: "suspended", feature: "posts" }),
    ).toBe(false);
  });
  it("expires grace at the deadline and denies missing deadlines", () => {
    expect(
      canUseFeature({ ...base, billingStatus: "grace", feature: "posts" }),
    ).toBe(false);
    expect(
      canUseFeature({
        ...base,
...
describe("hostname boundary", () => {
  it("normalizes DNS names and development ports", () => {
    expect(normalizeHostname("Creator.Example.com.:3000")).toBe(
      "creator.example.com",
    );
  ...
  it("rejects unverified, suspended and mismatched domain records", async () => {
    ...
    expect(
      await resolveTenant("a.com", async () => ({
        ...record,
        verified: false,
      })),
    ).toBeNull();
